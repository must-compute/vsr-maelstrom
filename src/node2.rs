...
impl Node2 {
    ...
    pub async fn run(self: Arc<Self>) -> tokio::sync::mpsc::Receiver<Message> {
        ...
        tokio::spawn(async move {
            while let Some(msg) = stdin_rx.recv().await {
                // std::mem::drop(unacked);

                let mut r: Option<tokio::sync::oneshot::Sender<Message>> = None;
                {
                    let mut unacked = self.unacked.lock().unwrap();
                    r = unacked.remove(&msg.body.msg_id());
                }

                if let Some(responder) = r {
    ...
    //             s.unacked.clone().lock().unwrap().remove(&msg.body.msg_id())
    //         {
    //             responder.send(msg).unwrap();
    //         } else {
    //             tx.send(msg).await.unwrap();
